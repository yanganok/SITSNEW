#ifndef LOGGING_FILE_APPENDER_HEADER_
#define LOGGING_FILE_APPENDER_HEADER_
...
namespace logging
{
	 ...
	 * property is set to true then logging uses OS specific
	 ...
	 * </dl>
	 */
	class LOGGING_EXPORT FileAppenderBase : public Appender {
	public:
		// Methods
		virtual void close();
	...
	protected:
		// Ctors
		FileAppenderBase(const logging::tstring& filename,
			std::ios_base::openmode mode = std::ios_base::trunc,
			bool immediateFlush = true,
			bool createDirs = false);
		FileAppenderBase(const logging::helpers::Properties& properties,
			std::ios_base::openmode mode = std::ios_base::trunc);
		...
		std::unique_ptr<logging::tchar[]> buffer;

		logging::tofstream out;
		logging::tstring filename;
		logging::tstring localeName;
		logging::tstring lockFileName;
		std::ios_base::openmode fileOpenMode;

		logging::helpers::Time reopen_time;
	 ...
	 */
	class LOGGING_EXPORT FileAppender : public FileAppenderBase
	{
	public:
		// Ctors
		FileAppender(const logging::tstring& filename,
			std::ios_base::openmode mode = std::ios_base::trunc,
			bool immediateFlush = true,
			bool createDirs = false);
		FileAppender(const logging::helpers::Properties& properties,
			std::ios_base::openmode mode = std::ios_base::trunc);
	 ...
	 * </dl>
	 */
	class LOGGING_EXPORT RollingFileAppender : public FileAppender {
	public:
		// Ctors
		RollingFileAppender(const logging::tstring& filename,
			long maxFileSize = 10 * 1024 * 1024,	// Size = 10 MB
			int maxBackupIndex = 1,					// Backup = 1
			bool immediateFlush = true,
			bool createDirs = false);

		RollingFileAppender(const logging::helpers::Properties& properties);
	...
	private:
		LOGGING_PRIVATE void init(long maxFileSize, int maxBackupIndex);
	};
	 ...
	 * format supported by {@link logging::helpers::Time::getFormatterTime()}</code>.
	 ...
	 * </dl>
	 */
	class LOGGING_EXPORT DailyRollingFileAppender : public FileAppender {
	public:
		// Ctors

		DailyRollingFileAppender(const logging::tstring& filename,
			DailyRollingFileSchedule schedule = DAILY,
			bool immediateFlush = true,
			int maxBackupIndex = 10,
			bool createDirs = false,
			bool rollOnClose = true,
			const logging::tstring& datePattern = logging::tstring());

		DailyRollingFileAppender(const logging::helpers::Properties& properties);
	...
	protected:
		virtual void append(const spi::InternalLoggingEvent& event);
		void rollover(bool alreadyLocked = false);
		logging::helpers::Time calculateNextRolloverTime(const logging::helpers::Time& t) const;
		logging::tstring getFilename(const logging::helpers::Time& t) const;

		// Data
		DailyRollingFileSchedule schedule;
		logging::tstring scheduledFilename;
		logging::helpers::Time nextRolloverTime;
		int maxBackupIndex;
		bool rollOnClose;
		logging::tstring datePattern;

	private:
		LOGGING_PRIVATE void init(DailyRollingFileSchedule schedule);
	};
	 ...
	 * </dl>
	 */
	class LOGGING_EXPORT TimeBasedRollingFileAppender : public FileAppenderBase {
	public:
		// Ctors
		TimeBasedRollingFileAppender(const tstring& filename = LOGGING_TEXT(""),
			const tstring& filenamePattern = LOGGING_TEXT("%d.log"),
			int maxHistory = 10,
	...
	protected:
		...
		logging::helpers::Time lastHeartBeat;
		logging::helpers::Time nextRolloverTime;
		bool rollOnClose;
...
#endif // LOGGING_FILE_APPENDER_HEADER_
