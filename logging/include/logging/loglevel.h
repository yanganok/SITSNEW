#ifndef LOGGING_LOGLEVEL_HEADER_
#define LOGGING_LOGLEVEL_HEADER_
...
namespace logging {
	 ...
	 */
	typedef logging::tstring const & (*LogLevelToStringMethod)(LogLevel);
	 ...
	 */
	typedef LogLevel(*StringToLogLevelMethod)(const logging::tstring&);
	 ...
	 * </ol>
	 */
	class LOGGING_EXPORT LogLevelManager {
	public:
		LogLevelManager();
		~LogLevelManager();
		 ...
		 */
		logging::tstring const & toString(LogLevel ll) const;

		/**
		 * This method is called by all classes internally to logging to
		 * convert a string into a LogLevel.
		 *
		 * Note: It traverses the list of <code>StringToLogLevelMethod</code>
		 *       to do this, so all "derived" LogLevels are recognized as well.
		 */
		LogLevel fromString(const logging::tstring& arg) const;
	 ...
	 */
	LOGGING_EXPORT LogLevelManager& getLogLevelManager();

}


#endif // LOGGING_LOGLEVEL_HEADER_
