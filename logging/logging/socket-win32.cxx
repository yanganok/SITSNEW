...
#if defined (LOGGING_USE_WINSOCK)
...
namespace
{
	...
	static
		...
		init_winsock_worker()
	{
		logging::helpers::LogLog * loglog
			= logging::helpers::LogLog::getLogLog();
		...
		case WS_UNINITIALIZED:
		{
			...
			if (ret != 0)
			{
				...
				loglog->error(LOGGING_TEXT("Could not initialize WinSock."),
					true);
			}
		...
		case WS_INITIALIZING:
			...
			while (true)
			{
				...
				case WS_INITIALIZING:
					logging::thread::yield();
					continue;

				default:
					assert(0);
					loglog->error(LOGGING_TEXT("Unknown WinSock state."), true);
				}
			}
		...
		init_winsock()
	{
		...
		// and exchange.
		if (LOGGING_LIKELY(winsock_state == WS_INITIALIZED))
			return;
		else
			init_winsock_worker();
	}
...
namespace logging {
	namespace helpers {
			...
			acceptSocket(SOCKET_TYPE sock, SocketState & state)
		{
			...
			return to_LOGGING_socket(connected_socket);
		}
			...
			getHostname(bool fqdn)
		{
				...
				else
				{
					...
					else
					{
						helpers::getLogLog().error(
							LOGGING_TEXT("Failed to get own hostname. Error: ")
			...
			ret = GetAddrInfo(LOGGING_C_STR_TO_TSTRING(hostname).c_str(), nullptr,
				&addr_info_hints, &ai);
			if (ret != 0)
			{
				WSASetLastError(ret);
				helpers::getLogLog().error(
					LOGGING_TEXT("Failed to resolve own hostname. Error: ")
