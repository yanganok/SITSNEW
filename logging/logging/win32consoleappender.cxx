...
/* list of available colors which can be OR'ed together and provided as an INT in the config file, e.g.:
	   logging.appender.INFO_MSGS.TextColor=36
...
namespace logging
{
	...
	Win32ConsoleAppender::Win32ConsoleAppender(
		...
		, text_color(0)
	{
		properties.getBool(alloc_console, LOGGING_TEXT("AllocConsole"));
		properties.getBool(log_to_std_err, LOGGING_TEXT("logToStdErr"));
		properties.getUInt(text_color, LOGGING_TEXT("TextColor"));
	}
		...
		Win32ConsoleAppender::append(spi::InternalLoggingEvent const & event)
	{
		...
		if (console_out == INVALID_HANDLE_VALUE)
		{
			helpers::getLogLog().error(
				LOGGING_TEXT("Win32ConsoleAppender::append")
				LOGGING_TEXT("- Unable to get STD_OUTPUT_HANDLE."));
			return;
		}
		...
		if (handle_type == FILE_TYPE_UNKNOWN && GetLastError() != NO_ERROR)
		{
			helpers::getLogLog().error(
				LOGGING_TEXT("Win32ConsoleAppender::append")
				LOGGING_TEXT("- Error retrieving handle type."));
			return;
		}
			...
			std::size_t str_len)
	{
		...
		do
		{
			...
			{
				helpers::getLogLog().error(
					LOGGING_TEXT("Win32ConsoleAppender::write_handle")
					LOGGING_TEXT("- WriteFile has failed."));
				return;
			}
			...
			std::size_t str_len)
	{
		...
		if (text_color)
		{
			...
			{
				helpers::getLogLog().error(
					LOGGING_TEXT("Win32ConsoleAppender::write_console:")
					LOGGING_TEXT(" GetConsoleScreenBufferInfo failed"));
			...
			if (!ret)
			{
				helpers::getLogLog().error(
					LOGGING_TEXT("Win32ConsoleAppender::write_console:")
					LOGGING_TEXT(" SetConsoleTextAttribute failed"));
			}
		}
	...
	output:;
		do
		{
			...
			if (!ret)
			{
				helpers::getLogLog().error(
					LOGGING_TEXT("Win32ConsoleAppender::write_console")
					LOGGING_TEXT("- WriteConsole has failed."));
				break;
			}
