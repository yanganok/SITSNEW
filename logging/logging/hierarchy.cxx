...
namespace logging
{
		...
		else
		{
			...
			if (!inserted)
			{
				helpers::getLogLog().error(
					LOGGING_TEXT("Hierarchy::getInstanceImpl()- Insert failed"),
					true);
			}
			...
			if (pnm_it != provisionNodes.end())
			{
				...
				{
					helpers::getLogLog().error(
						LOGGING_TEXT("Hierarchy::getInstanceImpl()- Delete failed"),
						true);
				}
			}
		...
		Hierarchy::updateParents(Logger const & logger)
	{
		...
		for (std::size_t i = name.find_last_of(LOGGING_TEXT('.'), length - 1);
			i != tstring::npos && i > 0;
			i = name.find_last_of(LOGGING_TEXT('.'), i - 1))
		{
			...
			if (it != loggerPtrs.end()) {
				...
				if (it2 != provisionNodes.end()) {
					...
					if (!tmp.second) {
						helpers::getLogLog().error(
							LOGGING_TEXT("Hierarchy::updateParents()- Insert failed"),
							true);
					}
				}
			} // end if Logger found
