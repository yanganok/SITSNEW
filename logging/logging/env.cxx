...
#ifdef LOGGING_HAVE_WCHAR_H
#include <wchar.h>
#endif

#ifdef LOGGING_HAVE_DIRECT_H
#include <direct.h>
#endif
...
namespace logging {
	namespace internal {
...
#if defined(_WIN32)
		tstring const dir_sep(LOGGING_TEXT("\\"));
#else
		tstring const dir_sep(LOGGING_TEXT("/"));
#endif
			...
			get_env_var(tstring & value, tstring const & name)
		{
			...
			case ENOMEM:
				helpers::getLogLog().error(
					LOGGING_TEXT("_dupenv_s failed to allocate memory"));
				throw std::bad_alloc();

			default:
				helpers::getLogLog().error(
					LOGGING_TEXT("_dupenv_s failed. Error: ")
...
#else
			char const * val
				= std::getenv(LOGGING_TSTRING_TO_STRING(name).c_str());
			if (val)
				value = LOGGING_STRING_TO_TSTRING(val);
		...
		{
			logging::tistringstream iss(str);
			logging::tstring word;
			...
			bool result = true;
			if (word == LOGGING_TEXT("true"))
				val = true;
			else if (word == LOGGING_TEXT("false"))
				val = false;
		...
		namespace
		{
			...
			struct path_sep_comp
			{
				...
				{
#if defined (_WIN32)
					return ch == LOGGING_TEXT('\\') || ch == LOGGING_TEXT('/');
#else
					return ch == LOGGING_TEXT('/');
#endif
				}
			};
			...
			is_drive_letter(tchar ch)
		{
			...
			return LOGGING_TEXT('A') <= dl && dl <= LOGGING_TEXT('Z');
		}
			...
			get_drive_cwd(tchar drive)
		{
			...
			std::unique_ptr<wchar_t, free_deleter> cstr(
				_wgetdcwd(drive - LOGGING_TEXT('A') + 1, 0, 0x7FFF));
			if (!cstr)
			{
				int const eno = errno;
				helpers::getLogLog().error(
					LOGGING_TEXT("_wgetdcwd: ")
			...
			std::unique_ptr<char, free_deleter> cstr(
				_getdcwd(drive - LOGGING_TEXT('A') + 1, 0, 0x7FFF));
			if (!cstr)
			{
				int const eno = errno;
				helpers::getLogLog().error(
					LOGGING_TEXT("_getdcwd: ")
					+ helpers::convertIntegerToString(eno),
					true);
			}
		...
		{
#if defined (WIN32)
			tstring result(0x8000, LOGGING_TEXT('\0'));
			...
			return LOGGING_STRING_TO_TSTRING(buf);
		...
		retry_recognition:;
			...
			if (comp_count >= 7
				&& components[0].empty()
				&& components[1].empty()
				&& components[2] == LOGGING_TEXT("?")
				&& (components[3].size() == 3
					&& helpers::toUpper(components[3][0]) == LOGGING_TEXT('U')
					&& helpers::toUpper(components[3][1]) == LOGGING_TEXT('N')
					&& helpers::toUpper(components[3][2]) == LOGGING_TEXT('C')))
			{
...
#  endif
...
#else
			if (mkdir(LOGGING_TSTRING_TO_STRING(dir).c_str(), 0777) == 0)
				...
				long ret)
		{
			...
			{
				loglog.debug(
					LOGGING_TEXT("Created directory ")
					+ path);
			}
			else
			{
				tostringstream oss;
				oss << LOGGING_TEXT("Failed to create directory ")
					<< path
					<< LOGGING_TEXT("; error ")
					<< ret;
