...
#if defined (LOGGING_USE_BSD_SOCKETS)
...
#ifdef LOGGING_HAVE_SYS_SOCKET_H
...
#if defined (LOGGING_HAVE_NETINET_TCP_H)
#include <netinet/tcp.h>
#endif

#if defined (LOGGING_HAVE_ARPA_INET_H)
#include <arpa/inet.h>
#endif

#if defined (LOGGING_HAVE_ERRNO_H)
#include <errno.h>
#endif

#ifdef LOGGING_HAVE_NETDB_H
#include <netdb.h>
#endif

#ifdef LOGGING_HAVE_FCNTL_H
#include <fcntl.h>
#endif
...
namespace logging {
	namespace helpers {
		...
		// from lockfile.cxx
		LOGGING_PRIVATE bool trySetCloseOnExec(int fd);
				...
				SocketState& state)
		{
			...
			retval = getaddrinfo(
				host.empty() ? nullptr : LOGGING_TSTRING_TO_STRING(host).c_str(),
			...
			{
				int const eno = errno;
				helpers::getLogLog().warn(LOGGING_TEXT("setsockopt() failed: ")
			...
			state = ok;
			return to_LOGGING_socket(sock_holder.detach());
		}
				...
				SocketState& state)
		{
			...
			retval = getaddrinfo(LOGGING_TSTRING_TO_STRING(hostn).c_str(),
				port_str.c_str(), &addr_info_hints, &ai);
			if (retval != 0)
			{
				set_last_socket_error(retval);
			...
			acceptSocket(SOCKET_TYPE sock, SocketState& state)
		{
			...
			return to_LOGGING_socket(clientSock);
		}
			...
			getHostname(bool fqdn)
		{
...
#if defined (LOGGING_HAVE_ENAMETOOLONG)
			...
			if (ret != 0 || (ret == 0 && !fqdn))
				return LOGGING_STRING_TO_TSTRING(hostname);
...
#if defined (LOGGING_HAVE_PIPE2) && defined (O_CLOEXEC)
			ret = pipe2(fds, O_CLOEXEC);
			if (ret != 0)
				goto error;

#elif defined (LOGGING_HAVE_PIPE)
			ret = pipe(fds);
			if (ret != 0)
				goto error;
...
#  error You are missing both pipe() or pipe2().
				...
				default:
					...
					if ((interrupt_pipe.revents & POLLIN) == POLLIN)
					{
						...
						helpers::getLogLog().debug(
							LOGGING_TEXT("ServerSocket::accept- ")
							LOGGING_TEXT("accept() interrupted by other thread"));
						...
						if (ret == -1)
						{
							...
							helpers::getLogLog().warn(
								LOGGING_TEXT("ServerSocket::accept- read() failed: ")
								+ helpers::convertIntegerToString(eno));
					...
					else if ((accept_fd.revents & POLLIN) == POLLIN)
					{
						helpers::getLogLog().debug(
							LOGGING_TEXT("ServerSocket::accept- ")
							LOGGING_TEXT("accepting connection"));
			...
			ServerSocket::interruptAccept()
		{
			...
			if (ret == -1)
			{
				int const eno = errno;
				helpers::getLogLog().warn(
					LOGGING_TEXT("ServerSocket::interruptAccept- write() failed: ")
					+ helpers::convertIntegerToString(eno));
			}
		}
